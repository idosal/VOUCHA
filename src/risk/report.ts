...
export function buildRiskReport(t: Telemetry | null): RiskReport {
  ...
  if (t.webdriver) signals.push("the browser identified itself as automated software");
  if (!t.turnstileOk) signals.push("the bot check (Turnstile) did not pass");
...
export function renderRiskReportMarkdown(report: RiskReport, t: Telemetry | null): string {
  ...
  lines.push(
    report.automationLikely
      ? "**This quiz looks like it was completed by a script, not a person.** Worth a second look before trusting the pass."
